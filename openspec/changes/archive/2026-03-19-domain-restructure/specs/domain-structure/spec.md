# domain-structure Specification (Delta)
...
### Requirement: Separate Model Package

**Reason:** Consolidated into `internal/domain/` for DDD-light architecture.

**Migration:**
- `internal/models/canonical/*` → `internal/domain/` (split by type)

### Requirement: Separate Error Package

**Reason:** Consolidated into `internal/domain/errors.go` for domain purity.

**Migration:**
- `internal/errors/*` → `internal/domain/errors.go`

### Requirement: Separate Validation Package

**Reason:** Consolidated into `internal/domain/` for domain purity.

**Migration:**
- `internal/validation/result.go` → `internal/domain/result.go`
- `internal/validation/validators.go` → `internal/domain/validation.go`
- `internal/validation/pipeline.go` → `internal/domain/validation.go` (merged)
- `internal/validation/opencode/` → `internal/domain/opencode/`
