# errors-operation-error Specification (delta)

> **Foundation unit.** `core.OperationError` is introduced in this slice (parallel to `core.NotFoundError` from slice-4). It formalizes per-operation errors that flow through `*core.PartialSuccessError` aggregations and through `output.FormatError`.
...
> **Why a new capability, not a delta to `errors-typed-errors`**: `OperationError` is net-new (no equivalent type in the base spec), and the migration to typed errors for per-file library errors is a behavioral surface change. The new capability gives consumers (`runAdd` Mode 2, the per-file error rendering, the partial-success aggregation) a single source of truth for the contract.
