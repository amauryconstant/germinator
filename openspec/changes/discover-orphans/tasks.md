...
- [ ] 1.1 Add `DiscoverSummary` struct with TotalScanned, TotalOrphans, TotalAdded, TotalSkipped, TotalFailed fields
- [ ] 1.2 Rename `DiscoverOrphan` to `OrphanInfo` with Path, Type, Name, Issue fields
- [ ] 1.3 Rename `DiscoverConflict` to `ConflictInfo` with Orphan and Issue fields
- [ ] 1.4 Add `AddSuccess` struct with Type, Name, Path fields
- [ ] 1.5 Update `DiscoverResult` to include Summary field
...
- [ ] 5.1 Add tests for recursive scanning with nested directories
- [ ] 5.2 Add tests for enhanced DiscoverResult structure
- [ ] 5.3 Add tests for batch mode with conflicts and errors
- [ ] 5.4 Run `mise run test` to verify all tests pass
