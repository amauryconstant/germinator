...
This change is **slice 1 of 9**: it lands only the foundation — three new packages, the `domain → core` rename, the `infrastructure/` flattening, and the supporting lint rules — so subsequent changes (2–9) can build incrementally without each one re-doing the mechanical work. No commands are migrated in this change; `main.go` is not rewired; legacy files are not deleted.

The full migration is described in `openspec/changes/scaffold-cli-foundation/design.md` (Decisions section); this change implements only the items below. See [tasks.md](tasks.md) sections 1.1–1.8 for the implementation sequence and the design Decisions 1–12 for rationale.

## What Changes

### New packages (zero behavior change yet)

- **NEW** `internal/iostreams/` — `IOStreams` struct (`In`, `Out`, `ErrOut`, `Verbosef`, `Logger` (gated on `GERMINATOR_DEBUG`), `Styles`, `IsStdoutTTY`, `IsInteractive`, `SetStdoutTTY`), `System()` + `Test()` constructors
...
- **NEW** `core.PartialSuccessError` and `core.InitializeError` (in `internal/core/errors.go`) — typed errors for partial-success flows (used by `init` in change-5). `InitializeError` carries per-resource failure info (`Ref`, `InputPath`, `OutputPath`, `Cause`); `PartialSuccessError` aggregates `Succeeded`/`Failed` counts plus a `[]InitializeError` slice. Both types are defined here because `cmdutil.ExitCodeFor` and `output.FormatError` need to recognize them for testability. `InitializeError` follows the existing builder pattern (`WithSuggestions`, `WithContext`, lowercase fields with getters)
...
- **`cli-factory`** — The `cmdutil.Factory` pattern with lazy `func() (T, error)` fields; the only DI mechanism in the new architecture. Replaces the eager `ServiceContainer` + mutable `CommandConfig`.
- **`iostreams`** — Centralized terminal I/O abstraction (`In`, `Out`, `ErrOut`, TTY detection, color/styles, `Verbosef`, `Logger` (gated on `GERMINATOR_DEBUG`), `SetStdoutTTY`, `Test()` constructor).
- **`output-formats`** — The `--output json|table|plain` flag and `Exporter` interface shared by all read-only commands. Built-in `JSONExporter` and `TableExporter`.
- **`command-options-pattern`** — The `NewCmdXxx(f *Factory, runF func(*XxxOptions) error)` + `runXxx(opts)` template.

### Modified (delta specs)

- **`dependency-injection`** — `ServiceContainer` is **removed**; replaced conceptually by `cmdutil.Factory` (the Factory itself is introduced in this change; the removal of all callers happens in change-7). `NewServiceContainer()` is **removed** in change-7.
- **`exit-codes`** — The seven exit codes (`0`–`6`) collapse to three (`0, 1, 2`). `ExitCodeFor(err)` lives in `cmdutil` and uses `errors.As` to map typed errors. The `Category*` enum and `CategorizeError` function are **removed** in change-7.
- **`framework`** — `CommandConfig` struct is **removed** in change-7; commands take `*cmdutil.Factory`. `RunE` populates an `Options` struct then delegates to `runXxx`. `runF` parameter enables test injection.
- **`verbose-output`** — The `Verbosity` type and `VerbosePrint` helpers are **removed** in change-7; replaced by `opts.IO.Verbosef(format, args...)` on `IOStreams`. `-v`/`-vv` flag semantics preserved.
- **`error-formatting`** — `ErrorFormatter` struct is **removed** in change-7; replaced by `output.FormatError(io, err)`. Type-specific formatters become private functions in `output/`.

> **Note:** Delta specs in this change are stored in flat folders under `specs/<name>/spec.md` (the `openspec` CLI requires this layout for delta discovery). When synced to `openspec/specs/`, each delta must be placed under the appropriate category folder per `openspec/specs/AGENTS.md`. The mapping for this change is:
>
> | Delta spec | Main-spec category |
> |------------|--------------------|
> | `cli-factory` | `cli/` |
> | `iostreams` | `cli/` |
> | `output-formats` | `cli/` |
> | `command-options-pattern` | `cli/` |
> | `dependency-injection` (delta) | `application/` |
> | `exit-codes` (delta) | `cli/` |
> | `framework` (delta) | `cli/` |
> | `verbose-output` (delta) | `cli/` |
> | `error-formatting` (delta) | `cli/` |
>
> This matches the layout of the existing main specs in `openspec/specs/`.
...
- **New (≈11 files):** `internal/iostreams/{iostreams,styles,iostreams_test,styles_test}.go`, `internal/output/{errors,exporter,output_flags,output_test}.go`, `internal/cmdutil/{factory,exit,output_flags,factory_test,exit_test}.go`, `internal/core/rules.go`
...
- **New (2 files):** `cmd/lint_test.go`, `cmd/testdata/lint_baseline.txt`
...
- **Large mechanical rename** — 12 files moved + ~15 files flattened + every import updated. **Mitigation:** `mise run check` runs after each major group of moves (tasks 1.2.4, 1.3.7) so the tree stays green between groups; `goimports -w -local gitlab.com/amoconst/germinator .` over the affected subtrees normalizes the import paths en masse.
...
- **Empty `infrastructure/` directory** — after flattening, the parent must be removed cleanly. **Mitigation:** task 1.3.8 explicitly removes the tree; checked with `find internal/infrastructure -type f` returning nothing.
