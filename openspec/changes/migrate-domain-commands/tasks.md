...
- [ ] 3.1.2 Declare the `Validator` interface in `cmd/validate.go` (one method: `Validate(ctx, *ValidateRequest) (*core.ValidateResult, error)`); import `ValidateRequest` from `internal/application/requests.go` (still alive in this change)
- [ ] 3.1.3 Implement `NewCmdValidate(f *cmdutil.Factory, runF func(*validateOptions) error) *cobra.Command`:
  - Add `--platform` string flag
  - In `RunE`: construct `opts`, populate from `f.IOStreams`, `c.Context()`, `f.Validator`, and parsed flags
...
- [ ] 3.2.2 Declare the `Canonicalizer` interface in `cmd/canonicalize.go` (one method: `Canonicalize(ctx, *CanonicalizeRequest) (*core.CanonicalizeResult, error)`)
...
- [ ] 3.2.5 Move `internal/service/canonicalizer_golden_test.go` to `cmd/canonicalize_golden_test.go` (or keep in `internal/service/` with updated package decl); ensure golden file tests still pass byte-identically
...
- [ ] 3.4.3 Run `mise run build` — confirm `bin/germinator` builds
- [ ] 3.4.4 Run `mise run test:coverage` — confirm coverage for `cmd/validate.go` and `cmd/canonicalize.go` ≥ 70%
- [ ] 3.4.5 Run `mise run test:e2e` — confirm E2E tests for validate and canonicalize pass
- [ ] 3.4.6 Smoke-test every command end-to-end (regression check from change-2):
...
- [ ] 3.4.7 Update `cmd/AGENTS.md` to include `validate` and `canonicalize` as examples of the new pattern (alongside adapt)
- [ ] 3.4.8 Confirm `legacyBridge` still works for non-migrated commands (init, library init, library add, library create, library refresh, library remove, library validate, config, completion, version)
