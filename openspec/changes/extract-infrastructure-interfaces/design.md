...
**Goals:**
- Enable unit testing of `Transformer`, `Canonicalizer`, and `Initializer` services without filesystem or YAML parsing
...
- Not adding new infrastructure interfaces beyond Parser and Serializer
...
**Rationale:** Minimal interface that covers service needs. Implementation can delegate to existing functions.
...
func NewServiceContainer() *ServiceContainer {
    ...
    return &ServiceContainer{
        ...
        Canonicalizer: service.NewCanonicalizer(parser, serializer),
        Initializer:   service.NewInitializer(parser, serializer),
    }
}
```

**Rationale:** Keeps ServiceContainer as composition root. Infrastructure implementations remain internal to infrastructure package.
...
[Risk] Two interfaces added but only Parser used by Validator
→ [Mitigation] Minimal change. Can add Serializer to Validator later if validation pipeline uses rendering.
...
3. Update service constructors to accept new interfaces
4. Update `ServiceContainer` to instantiate and inject infrastructure
5. Create mocks in `test/mocks/`
6. Add unit tests for services using mocks
...
1. Should Validator also receive Parser and Serializer for future flexibility, or keep as-is?
