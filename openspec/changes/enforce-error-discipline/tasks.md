...
- [ ] 0.1 Re-verify every file:line in this `tasks.md` against the current tree. For each site, run the listed `rg` and `sed -n` checks; record discrepancies in this commit's body and update the affected task descriptions before proceeding. Specifically:
  ...
  - `rg "errEmptyResources" cmd/` — must show a single declaration at `cmd/library_create.go:70` and a single call at `:203`.
  - `rg --line-number 'BatchFailureInfo\{' internal/library/` — must show 5 population sites.
  - `rg --line-number 'NewFileError.*not found' internal/library/` — must show exactly 5 sites (resolver.go:21,26; loader.go:36,53; adder.go:146) before migration; 0 after.
  - `rg --line-number 'NewConfigError.*preset not found' internal/library/` — must show exactly 1 site (resolver.go:70) before migration; 0 after.
  - `rg --line-number "os\.IsNotExist" internal/library/remover.go` — must show the silent-swallow site at line `104`.
...
- [ ] 1.2 In `internal/cmdutil/exit.go:24-37`, delete the `cobraUsagePrefixes` slice (12 entries) and the `hasCobraUsagePrefix` function. The existing typed branch (`errors.As(err, &pflag.NotExistError)` etc. at lines 64-69) handles the four pflag-emitted types directly. Remove the corresponding call to `hasCobraUsagePrefix` at line 70.
...
- [ ] 2.11 Run `rg "output\.FormatError" cmd/ main.go` — must return the 4 retained production CALLS (`main.go:32` factory-build handler, `main.go:46` post-Execute handler, `cmd/init_test.go:185` test renderer, `cmd/show_test.go:239` test renderer) plus comment-only references in `cmd/AGENTS.md`, `cmd/commands/AGENTS.md`, and various `cmd/*_test.go` files. Any additional production CALL sites in `cmd/` are missed deletes from tasks `2.1-2.10`.
...
- [ ] 4.3 Drop the `runF` parameter from `cmd/library.go:11` (`NewLibraryCommand`). Drop the `runF` parameter from `cmd/resources.go:48` (`NewCmdResources`). Update all 6 call sites: `cmd/library.go:39` (forward to `NewCmdResources`), `cmd/library_test.go:30,52,76,95` (4 test rows), `cmd/root.go:35`. Build must remain green; `cmd/resources_test.go` continues to construct `NewCmdResources` directly without `runF`.
...
- [ ] 4.5 Update `cmd/library_init.go:161-165` to set the new `InitRequest.Stdout io.Writer` field introduced by `openspec/changes/fix-library-io-discipline/tasks.md:1.4`. Pass `opts.IO.Out` to preserve the current stderr-as-OutForCobra semantics. **Dependency**: this task lands AFTER `fix-library-io-discipline` Phase 1 ships, so the `InitRequest` struct is final. The new field is additive (existing call sites that pass `nil` for `Stdout` fall through to `os.Stdout`).
- [ ] 4.6 Cross-references — update `openspec/changes/harden-tests-and-coverage/tasks.md:6.6` to mark the `errEmptyResources` migration as deferred (this change owns it). Update `openspec/changes/fix-library-io-discipline/proposal.md` Risks section to note that this change consumes Phase 1's `stdout` parameter.
...
## 6. Phase 6 — Archive + CHANGELOG

- [ ] 6.0 **CHANGELOG FIRST** (the change cannot archive without this). Append the entries to the existing `### BREAKING` / `### Changed` blocks at the top of the `## [Unreleased]` section of `/CHANGELOG.md` (project uses Keep-a-Changelog format per the existing file):
  - `### BREAKING`: `*core.NotFoundError` now maps to exit code `1` (operational error) instead of exit code `2` (usage error). Scripts that special-case `exit 2` for not-found scenarios must update to `exit 1`.
  - `### Removed`: The exit-code deprecation canary (`internal/warning.MaybeWarnLegacyExitCode`, the entire `internal/warning` package) has been removed. The exit-code 5 → 1 migration it was added to support (in `migrate-library-rest` slice 7) is now complete.
  - `### Changed`: `cmd library create preset --resources ""` now prints `Error: --resources: must be non-empty list of refs` instead of `Error: flag needs an argument: --resources ...`. Exit code remains `2`.
  - `### Changed`: `*core.*Error` types now implement `json.Marshaler`. JSON serialization of these errors returns `{"error": "<Error()>"}` instead of `{}`. Any future exported struct fields on these types must be exposed via `MarshalJSON` to appear in JSON output.
  - `### Added`: New `*core.UsageError` typed error for CLI flag validation errors not caught by `cobra.MarkFlagRequired` or `Args` validators; carries `flag`, `reason`, and `suggestions` (private) with `WithSuggestions(...)` immutable builder; maps to exit code `2`. `*core.CobraUsageError` sentinel for typed dispatch against Cobra arg-validation errors; constructed via `MustNewCobraUsageError(err)` which panics on `nil` cause.
  - `### Added`: `internal/library.BatchFailureInfo` gains `ErrorType string` (`json:",omitempty"`) and `Cause error` (`json:",omitempty"`) fields for typed-error chain preservation. `ErrorType` is computed via a typed switch in `internal/library/adder.go` mapping well-known types to canonical names.
  - `### Added`: New `*config.WriteError` domain type in `internal/config/errors.go` for I/O failures during config scaffolding (e.g., `WriteDefault` path). Returns are surfaced via `internal/config/scaffold.go WriteDefault(path, force) error`. Maps to exit code `1` (operational error). The `case *config.WriteError` arm of `output.FormatError` ships with the type introduction (Phase 1 task `1.5b`).
  - `### Added`: New `core.ValidateDocumentType(docType string) error` helper in `internal/core/rules.go` for canonicalize `--type` pre-flight validation.
- [ ] 6.1 Archive this change via `osc-archive-change enforce-error-discipline`.
- [ ] 6.2 Confirm `openspec list --json` shows the change under `archive/` with `status: archived`.
- [ ] 6.3 Confirm `openspec list --json` shows `harden-tests-and-coverage` task `6.6` marked as deferred to this change (the cross-reference note).
