...
- Fold A-009 (`config_init` → `internal/config/scaffold.go` + new `*config.WriteError` domain type), A-011 (`--type` validation via `core.ValidateDocumentType` defense-in-depth pre-flight in `runCanonicalize`; `MarkFlagRequired("type")` stays unchanged). **A-007 (drop `runF` from `NewLibraryCommand` and `NewCmdResources`) is deferred to a follow-up change** — see the Non-Goals block below.
- Set the `InitRequest.Stdout io.Writer` field on the `library.InitRequest` literal at `cmd/library_init.go:161-165` to `opts.IO.Out`. The field is available now (`fix-library-io-discipline` has shipped); the consumption edit is an unconditional code update.
...
- **Removing the `runF` parameter from `NewLibraryCommand` (`cmd/library.go:11`) and `NewCmdResources` (`cmd/resources.go:48`)** — deferred to a follow-up change. The canonical `golang-cli-architecture` Factory pattern treats `runF` as the test-injection seam (see the Factory section in `golang-cli-architecture/SKILL.md`); closing it requires a concrete audit confirming the seam is unused across all current and planned test files for these two constructors. Until that audit is performed, the removal risks closing a future-test affordance.
...
- **Cross-change ownership.** This change claims ownership of the `errEmptyResources` migration (task `3.12`) referenced in `openspec/changes/harden-tests-and-coverage/proposal.md:59`. **Mitigation**: explicit cross-reference in `harden-tests-and-coverage/tasks.md` de-scopes the original task; no other cross-change dependencies remain (the `fix-library-io-discipline` change has shipped and `task 4.5` is an unconditional code update).
...
5. **Phase 4 — Trivial folds + cross-change imports** (tasks `4.1-4.6`): create `internal/config.WriteDefault`; refactor `cmd/config_init.go:144-156`; **drop `runF` from `NewLibraryCommand` and `NewCmdResources` is DEFERRED to a follow-up change (see Non-Goals)**; replace `MarkFlagRequired("type")` in `cmd/canonicalize.go:96` with `MatchAll(ExactArgs, OnlyValidArgs)` + pre-flight; set the `InitRequest.Stdout` field on `cmd/library_init.go:161-165` to `opts.IO.Out` (the field is now available — `fix-library-io-discipline` has shipped); cross-reference `harden-tests-and-coverage` task `6.6`. Verify `mise run check`.
