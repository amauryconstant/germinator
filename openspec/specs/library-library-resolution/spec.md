# Capability: Library Resolution

## Purpose

The Library Resolution capability provides lookup of preset resource references by preset name on an already-loaded `*library.Library` instance. It is the in-memory companion to library persistence and preset creation: once `LoadLibrary` has populated `Library.Presets`, callers resolve a preset's resource list with `lib.ResolvePreset(ctx, name)`.
