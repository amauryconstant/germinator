...
- `ParsePlatformDocument(ctx context.Context, path, platform, docType string) (interface{}, error)` — checks `ctx.Err()` then reads a platform YAML file, selects the adapter via `claudecode.ClaudeCode` / `opencode.OpenCode` (package-level singletons), calls `ToCanonical`, and wraps the result in the corresponding `Canonical*` struct.
