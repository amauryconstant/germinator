...
# Add the folder containing germinator.exe to your PATH
```
...
Invoke-WebRequest -Uri "https://gitlab.com/amoconst/germinator/-/releases/0.3.0/downloads/checksums.txt" -OutFile "checksums.txt"

# Verify the checksum (requires certutil)
certutil -hashfile germinator.zip SHA256
...
```
germinator 0.3.0 (abc1234) 2025-01-13
```
...
```
germinator_0.3.0_sbom.spdx.json
```
...
```powershell
# Remove the folder containing germinator.exe from your PATH, then delete the file
Remove-Item path\to\germinator.exe
```
